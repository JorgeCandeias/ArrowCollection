...
/// <summary>
/// Incremental source generator that generates optimized FrozenArrow implementations
/// for types marked with [ArrowRecord] attribute.
/// </summary>
[Generator]
public sealed class FrozenArrowGenerator : IIncrementalGenerator
{
    private const string ArrowRecordAttributeName = "FrozenArrow.ArrowRecordAttribute";
    private const string ArrowArrayAttributeName = "FrozenArrow.ArrowArrayAttribute";
    ...
    private static void Execute(SourceProductionContext context, ImmutableArray<ArrowRecordInfo> arrowRecords)
    {
        ...
        {
            var source = GenerateFrozenArrowImplementation(record);
            context.AddSource($"FrozenArrow_{record.ClassName}.g.cs", SourceText.From(source, Encoding.UTF8));
        }

        // Generate the factory registry for this assembly
        var registrySource = GenerateFactoryRegistry(validRecords);
        context.AddSource("FrozenArrowFactoryRegistry.g.cs", SourceText.From(registrySource, Encoding.UTF8));

        // Generate the module initializer to register factories
        var moduleInitializerSource = GenerateModuleInitializer(validRecords);
        context.AddSource("FrozenArrowModuleInitializer.g.cs", SourceText.From(moduleInitializerSource, Encoding.UTF8));
    }

    private static string GenerateFrozenArrowImplementation(ArrowRecordInfo record)
    {
        ...
        // Generate the concrete FrozenArrow implementation
        sb.AppendLine($"{indent}internal sealed class GeneratedFrozenArrow_{record.ClassName} : global::FrozenArrow.FrozenArrow<{record.FullTypeName}>");
        ...
        for (int i = 0; i < record.Fields.Count; i++)
        {
            ...
            {
                sb.AppendLine($"{indent}    private static readonly global::FrozenArrow.RefFieldSetter<{record.FullTypeName}, {field.FieldTypeName}> _setter{i} = GetSetter{i}();");
            }
            else
            {
        ...
        for (int i = 0; i < record.Fields.Count; i++)
        {
            ...
            if (record.IsValueType)
            {
                sb.AppendLine($"{indent}    private static global::FrozenArrow.RefFieldSetter<{record.FullTypeName}, {field.FieldTypeName}> GetSetter{i}()");
                sb.AppendLine($"{indent}    {{");
                ...
                sb.AppendLine($"{indent}        return global::FrozenArrow.FieldAccessor.GetRefSetter<{record.FullTypeName}, {field.FieldTypeName}>(fieldInfo);");
                sb.AppendLine($"{indent}    }}");
            }
            else
            {
                ...
                sb.AppendLine($"{indent}    {{");
                sb.AppendLine($"{indent}        var fieldInfo = typeof({record.FullTypeName}).GetField(\"{escapedBackingFieldName}\", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public)!;");
                sb.AppendLine($"{indent}        return global::FrozenArrow.FieldAccessor.GetSetter<{record.FullTypeName}, {field.FieldTypeName}>(fieldInfo);");
                sb.AppendLine($"{indent}    }}");
            }
            sb.AppendLine();
        }
        ...
        sb.AppendLine($"{indent}    internal GeneratedFrozenArrow_{record.ClassName}(RecordBatch recordBatch, int count, global::FrozenArrow.FrozenArrowBuildStatistics? buildStatistics = null)");
        ...
        sb.AppendLine($"{indent}    internal GeneratedFrozenArrow_{record.ClassName}(RecordBatch recordBatch, int count, int[] colIndex)");
        ...
        sb.AppendLine();
        sb.AppendLine($"{indent}internal static class FrozenArrowBuilder_{record.ClassName}");
        sb.AppendLine($"{indent}{{");
        ...
        for (int i = 0; i < record.Fields.Count; i++)
        {
            ...
            sb.AppendLine($"{indent}    {{");
            sb.AppendLine($"{indent}        var fieldInfo = typeof({record.FullTypeName}).GetField(\"{escapedBackingFieldName}\", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public)!;");
            sb.AppendLine($"{indent}        return global::FrozenArrow.FieldAccessor.GetGetter<{record.FullTypeName}, {field.FieldTypeName}>(fieldInfo);");
            sb.AppendLine($"{indent}    }}");
            sb.AppendLine();
        }


        sb.AppendLine($"{indent}    internal static global::FrozenArrow.FrozenArrow<{record.FullTypeName}> Create(global::System.Collections.Generic.IEnumerable<{record.FullTypeName}> source)");
        sb.AppendLine($"{indent}    {{");
        ...
        for (int i = 0; i < record.Fields.Count; i++)
        {
            ...
            sb.AppendLine($"{indent}        var statsCollector{i} = new global::FrozenArrow.ColumnStatisticsCollector<{field.FieldTypeName}>(\"{field.MemberName}\");");
        }
        sb.AppendLine();
        ...
        sb.AppendLine($"{indent}        var columnStats = new global::System.Collections.Generic.Dictionary<string, global::FrozenArrow.ColumnStatistics>");
        sb.AppendLine($"{indent}        {{");
        for (int i = 0; i < record.Fields.Count; i++)
        {
            var field = record.Fields[i];
        ...
        sb.AppendLine();
            sb.AppendLine($"{indent}        var buildStatistics = new global::FrozenArrow.FrozenArrowBuildStatistics");
            sb.AppendLine($"{indent}        {{");
            ...
            sb.AppendLine();
            sb.AppendLine($"{indent}        return new GeneratedFrozenArrow_{record.ClassName}(recordBatch, count, buildStatistics);");
            sb.AppendLine($"{indent}    }}");
            sb.AppendLine();
        ...
        private static void GenerateCreateFromRecordBatchMethod(StringBuilder sb, ArrowRecordInfo record, string indent)
        {
            ...
            sb.AppendLine($"{indent}    /// Creates a FrozenArrow from a deserialized RecordBatch.");
            sb.AppendLine($"{indent}    /// </summary>");
            sb.AppendLine($"{indent}    internal static global::FrozenArrow.FrozenArrow<{record.FullTypeName}> CreateFromRecordBatch(global::Apache.Arrow.RecordBatch recordBatch, global::FrozenArrow.ArrowReadOptions options)");
            sb.AppendLine($"{indent}    {{");
            ...
            sb.AppendLine($"{indent}        if (options.UnknownColumns == global::FrozenArrow.UnknownColumnBehavior.Throw)");
            sb.AppendLine($"{indent}        {{");
            ...
            sb.AppendLine($"{indent}        if (options.MissingColumns == global::FrozenArrow.MissingColumnBehavior.Throw)");
            sb.AppendLine($"{indent}        {{");
            ...
            sb.AppendLine($"{indent}        return new GeneratedFrozenArrow_{record.ClassName}(recordBatch, recordBatch.Length, colIndex: new int[] {{ {string.Join(", ", Enumerable.Range(0, record.Fields.Count).Select(i => $"colIndex{i}"))} }});");
            sb.AppendLine($"{indent}    }}");
        }
    ...
    private static void GenerateOptimalStringArrayBuilder(StringBuilder sb, ArrowFieldInfo field, int index, string indent, string stats, string nullable)
    {
        ...
        sb.AppendLine($"{indent}{{");
        sb.AppendLine($"{indent}    var array{index} = global::FrozenArrow.RunLengthEncodedArrayBuilder.BuildStringArray(values{index}, {stats}, allocator);");
    ...
    private static void GenerateOptimalInt32ArrayBuilder(StringBuilder sb, ArrowFieldInfo field, int index, string indent, string stats, string nullable)
    {
        ...
        sb.AppendLine($"{indent}{{");
        sb.AppendLine($"{indent}    var array{index} = global::FrozenArrow.RunLengthEncodedArrayBuilder.BuildInt32Array(values{index}, {stats}, allocator);");
        sb.AppendLine($"{indent}    arrays.Add(array{index});");
        sb.AppendLine($"{indent}    schemaFields.Add(new Field(\"{field.ColumnName}\", array{index}.Data.DataType, {nullable}));");
        sb.AppendLine($"{indent}}}");
    }
    ...
    private static void GenerateOptimalDoubleArrayBuilder(StringBuilder sb, ArrowFieldInfo field, int index, string indent, string stats, string nullable)
    {
        ...
        sb.AppendLine($"{indent}{{");
        sb.AppendLine($"{indent}    var array{index} = global::FrozenArrow.RunLengthEncodedArrayBuilder.BuildDoubleArray(values{index}, {stats}, allocator);");
        sb.AppendLine($"{indent}    arrays.Add(array{index});");
        sb.AppendLine($"{indent}    schemaFields.Add(new Field(\"{field.ColumnName}\", array{index}.Data.DataType, {nullable}));");
        sb.AppendLine($"{indent}}}");
    }
    ...
    private static void GenerateOptimalDecimalArrayBuilder(StringBuilder sb, ArrowFieldInfo field, int index, string indent, string stats, string nullable)
    {
        ...
        sb.AppendLine($"{indent}{{");
        sb.AppendLine($"{indent}    var array{index} = global::FrozenArrow.RunLengthEncodedArrayBuilder.BuildDecimalArray(values{index}, {stats}, allocator);");
        sb.AppendLine($"{indent}    arrays.Add(array{index});");
        sb.AppendLine($"{indent}    schemaFields.Add(new Field(\"{field.ColumnName}\", array{index}.Data.DataType, {nullable}));");
        sb.AppendLine($"{indent}}}");
    }
    ...
    private static string GenerateFactoryRegistry(List<ArrowRecordInfo> arrowRecords)
    {
        ...
        sb.AppendLine("#nullable enable");
        sb.AppendLine();
        sb.AppendLine("namespace FrozenArrow");
        sb.AppendLine("{");
        sb.AppendLine("    internal static partial class GeneratedFrozenArrowFactories");
        sb.AppendLine("    {");
        ...
        foreach (var record in arrowRecords)
        {
            ...
            sb.AppendLine($"            {{ typeof({record.FullTypeName}), new global::System.Func<global::System.Collections.Generic.IEnumerable<{record.FullTypeName}>, global::FrozenArrow.FrozenArrow<{record.FullTypeName}>>({builderNamespace}FrozenArrowBuilder_{record.ClassName}.Create) }},");
        }

        sb.AppendLine("        };");
        sb.AppendLine();
        sb.AppendLine("        internal static bool TryGetFactory<T>(out global::System.Func<global::System.Collections.Generic.IEnumerable<T>, global::FrozenArrow.FrozenArrow<T>>? factory) where T : new()");
        sb.AppendLine("        {");
        sb.AppendLine("            if (_factories.TryGetValue(typeof(T), out var factoryDelegate))");
        sb.AppendLine("            {");
        sb.AppendLine("                factory = (global::System.Func<global::System.Collections.Generic.IEnumerable<T>, global::FrozenArrow.FrozenArrow<T>>)factoryDelegate;");
    ...
    private static string GenerateModuleInitializer(List<ArrowRecordInfo> arrowRecords)
    {
        ...
        sb.AppendLine("namespace FrozenArrow");
        sb.AppendLine("{");
        sb.AppendLine("    internal static class FrozenArrowModuleInitializer");
        sb.AppendLine("    {");
        ...
        {
            var builderNamespace = record.Namespace is not null ? $"global::{record.Namespace}." : "global::";
            sb.AppendLine($"            global::FrozenArrow.FrozenArrowFactoryRegistry.Register<{record.FullTypeName}>({builderNamespace}FrozenArrowBuilder_{record.ClassName}.Create);");
            sb.AppendLine($"            global::FrozenArrow.FrozenArrowFactoryRegistry.RegisterDeserialization<{record.FullTypeName}>({builderNamespace}FrozenArrowBuilder_{record.ClassName}.CreateFromRecordBatch);");
        }
