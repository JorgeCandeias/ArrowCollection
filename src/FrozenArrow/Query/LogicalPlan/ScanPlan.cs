    ...
    long rowCount) : LogicalPlanNode
{
    ...
    /// </summary>
    public object SourceReference { get; } = sourceReference ?? throw new ArgumentNullException(nameof(sourceReference));

    public override string Description => $"Scan({TableName})";

    public override long EstimatedRowCount { get; } = rowCount;

    public override IReadOnlyDictionary<string, Type> OutputSchema { get; } = schema ?? throw new ArgumentNullException(nameof(schema));
