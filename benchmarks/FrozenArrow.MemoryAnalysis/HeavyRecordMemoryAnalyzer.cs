...
public static class HeavyRecordMemoryAnalyzer
{
    ...
    private static void WarmUp()
    {
        ...
        using var warmupCollection = warmupItems.ToFrozenArrow();
    ...
    private static void RunEmpiricalAnalysis(string scenarioTitle, bool usePreSorting)
    {
        ...
        // Measure FrozenArrow using process memory
        if (usePreSorting)
        {
            Console.WriteLine("Measuring FrozenArrow memory footprint (PRE-SORTED by cardinality)...");
            Console.WriteLine("  (Sorting: String columns first -> Sparse columns -> High-cardinality columns)");
        }
        else
        {
            Console.WriteLine("Measuring FrozenArrow memory footprint (RANDOM order)...");
        }
        Console.WriteLine("  (Generating 1M sparse items and converting to FrozenArrow...)");
        ...
        FrozenArrow<HeavyMemoryTestItem> FrozenArrow;
        if (usePreSorting)
        {
            // Pre-sort by cardinality: lowest first (strings), then sparse numerics, then high-cardinality
            FrozenArrow = GenerateHeavyItemsEnumerable(ItemCount)
        ...
        {
            FrozenArrow = GenerateHeavyItemsEnumerable(ItemCount).ToFrozenArrow();
        }
        ...
        Console.WriteLine($"  FrozenArrow process memory: {arrowMB:F2} MB ({arrowGB:F4} GB)");
        Console.WriteLine();

        // Display build statistics
        if (FrozenArrow.BuildStatistics is not null)
        {
            Console.WriteLine("BUILD STATISTICS:");
            Console.WriteLine("-----------------");
            var stats = FrozenArrow.BuildStatistics;
        ...
        Console.WriteLine($"|  FrozenArrow:        {arrowMB,10:F2} MB ({arrowGB:F4} GB)       |");
        Console.WriteLine("+------------------------------------------------------------------+");
        Console.WriteLine($"|  Memory savings:         {savingsMB,10:F2} MB ({savingsPercent:F1}%)          |");
        Console.WriteLine("+==================================================================+");
        Console.WriteLine();

        GC.KeepAlive(FrozenArrow);
        FrozenArrow.Dispose();
        ForceFullGC();
        Thread.Sleep(200);
    }
